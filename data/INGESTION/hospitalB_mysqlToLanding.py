...
GCS_BUCKET = "healthcare-bucket-15032026"
...
# MySQL Configuration
MYSQL_CONFIG = {
    "url": "jdbc:mysql://35.239.113.1:3306/hospital_b_db?useSSL=true&allowPublicKeyRetrieval=true",
...
def write_df_to_gcs_as_single_json(df, table):
    ...
    Spark writes to a temp directory as partitioned files; we then rename the
    single part file to the final clean filename and delete the temp directory.
    """
    today = datetime.datetime.today().strftime('%d%m%Y')

    temp_path = f"gs://{GCS_BUCKET}/temp/{HOSPITAL_NAME}/{table}/{table}_{today}_temp"
    final_blob_name = f"landing/{HOSPITAL_NAME}/{table}/{table}_{today}.json"
    temp_prefix = f"temp/{HOSPITAL_NAME}/{table}/{table}_{today}_temp/"

    # Write as single partition directly from workers — no driver memory used
    df.coalesce(1).write.mode("overwrite").json(temp_path)

    # Find the part file Spark produced and copy it to the final clean path
    temp_blobs = list(storage_client.bucket(GCS_BUCKET).list_blobs(prefix=temp_prefix))
    part_file = next((b for b in temp_blobs if b.name.endswith(".json")), None)

    if part_file is None:
        raise FileNotFoundError(f"No part JSON file found in temp path: {temp_prefix}")
    ...
    return f"gs://{GCS_BUCKET}/{final_blob_name}"
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        ...
        audit_df = spark.createDataFrame([
            ("hospital_b_db", table, load_type, record_count, datetime.datetime.now(), "SUCCESS")],
